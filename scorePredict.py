...
def run():
    ...
    currentGame = raw_data[gamesPlayedIndex:gamesPlayedIndex + 1]
    print "\n----------------\n\n    " + currentGame['Home'].values[0] + " vs " + currentGame['Away'].values[0]
    ...
    print "\nRandom-Forest: \n\t", int(round(rf_home_pred[0])), "-", int(
        round(rf_away_pred[0])), "\n w/o round: ", round(
        rf_home_pred[0], 2), "-", round(rf_away_pred[0], 2)
    ...
    print "\t" + str(f_home) + " - " + str(f_away), "\n w/o round: ", str(round(nn_home[0][0], 2)), "-", str(
        round(nn_away[0][0], 2))
